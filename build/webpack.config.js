...
require('./routerIndex-generater/index-generater')
...
files = fs.readdirSync(path.resolve(src_path,'./webStatic/bundles/'));
files.forEach((file, index) => {
    if(file.indexOf('.manifest.json') != -1){
        DllReferencePlugins.push(new webpack.DllReferencePlugin({
            ...
            context: '.',
            //引入生成的manifest.json。
            manifest: require( path.resolve(src_path,'./webStatic/bundles/'+ file ) ),
        }))
    }
});*/


var files = fs.readdirSync(path.resolve(src_path,'webStatic/bundles/'));
...
for(var file of files){
    ...
    if(file.indexOf('.json')!= -1){
        ...
        bundles.push('/webStatic/bundles/'+ file )
    }
